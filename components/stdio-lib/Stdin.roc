...
Stdin :: [].{
	...
	##
	## > Reading key-by-key also needs the terminal out of canonical mode, which
	## is termios state rather than a CLI primitive and lives in a separate
	## terminal package. Without it the tty still buffers by line, so this
	## returns nothing until Enter.
