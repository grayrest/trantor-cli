...
## Construct and operate on byte-preserving paths. Native Unix bytes and
## Windows UTF-16 units are preserved across host effects; use `display` only
## when a lossy human-readable representation is appropriate.
##
## The tags are basic-cli 0.21's `Unix`/`Windows`, not `OsStr`'s
## `UnixBytes`/`WindowsU16s`: `:=` lets an app name them, so renaming them
## breaks basic-cli programs that build or match a `Path` by tag.
## `to_raw`/`from_raw` rename at the boundary, as upstream does.
Path := [
	Utf8(Str),
	Unix(List(U8)),
	Windows(List(U16)),
